...
export function Notification({ notice, onDismiss }: { notice: LogEntry | null; onDismiss: () => void }) {
  return (
    <div className="notification-region" aria-live="polite" aria-atomic="true">
      {notice && (
        <div className={`notification ${notice.type === 'error' ? 'notification-error' : ''}`} key={`${notice.timestamp}:${notice.message}`}>
          <div className="min-w-0 flex-1">
            <p className="font-semibold">{notice.type === 'error' ? 'Action needs attention' : 'Device update'}</p>
