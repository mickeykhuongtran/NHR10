...
export const HistoryTab: React.FC<HistoryTabProps> = ({ 
  ...
  const handleShare = async () => {
    ...
    const formattedString = historyData.map(batch => {
      let batchString = '';
      Object.entries(batch).forEach(([key, value]) => {
        if (Array.isArray(value)) {
           batchString += `${key}:\n${value.map(epc => `- ${epc}`).join('\n')}\n`;
        } else if (typeof value === 'object' && value !== null) {
           batchString += `${key}: ${JSON.stringify(value)}\n`;
        } else {
           batchString += `${key}: ${value}\n`;
        }
      });
      return batchString;
    }).join('\n----------------------------------------\n\n');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-wider">
              {hasData ? `${historyData.length} RECORDS FOUND` : 'NO DATA LOADED'}
            </p>
          </div>
        </div>
        ...
        {!hasData && !isFileTransferring && (
          <div className="text-center space-y-6 max-w-sm">
            ...
            <div className="space-y-2">
              <h3 className="text-slate-300 font-bold">No Data Loaded</h3>
              <p className="text-slate-500 text-xs">
                Fetch data from the device's internal memory to preview and download batch scan results.
              </p>
            </div>
            <Button 
              fullWidth 
            ...
            >
              <RefreshCw size={18} className="mr-2" /> FETCH DATA FROM DEVICE
            </Button>
          </div>
        )}
        ...
        {isFileTransferring && (
          <div className="w-full max-w-sm bg-slate-900 p-6 rounded-lg border border-slate-800 shadow-xl space-y-4 text-center">
            ...
            <div className="space-y-1">
              <h3 className="text-slate-200 font-bold">Transferring Data...</h3>
              <p className="text-slate-500 text-xs font-mono">Please wait while we retrieve the file.</p>
            </div>
            <div className="space-y-2">
        ...
        {hasData && !isFileTransferring && (
          <div className="w-full h-full flex flex-col bg-slate-900 rounded-lg border border-slate-800 overflow-hidden shadow-inner">
            ...
            <div className="p-2 bg-slate-950 border-t border-slate-800 text-[10px] text-slate-600 font-mono text-center">
              Previewing {historyData.length} records
            </div>
          </div>
        )}
