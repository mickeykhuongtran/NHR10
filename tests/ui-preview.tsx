...
function Preview() {
  ...
  const [writeStatus, setWriteStatus] = useState<'idle' | 'success'>('idle');
  const [history, setHistory] = useState<{ INDEX: number; EPC: string }[]>([]);
  const log = (message: string, type: LogEntry['type'] = 'info') => setLogs(current => [...current, { message, type, timestamp: Date.now() }].slice(-1000));
  useEffect(() => {
    if (!connected) return;
  ...
  const props: React.ComponentProps<typeof DashboardLayout> = {
    status: connected ? 'connected' : 'disconnected', commandPending: false,
    ...
    onLocate: () => setLocating(true), onStopLocate: () => setLocating(false), targetRssi: locating ? -63 : null, isLocating: locating,
    onWriteEpc: () => { setWriteStatus('success'); log('Simulated write success'); }, onWriteData: () => setWriteStatus('success'), writeStatus, writeMessage: writeStatus === 'success' ? 'Simulated acknowledgement' : '',
  ...
  return <><div style={{ height: 36, padding: '8px 16px', background: '#fffbeb', color: '#92400e', fontSize: 12 }}>UI test fixture · Simulated data · No Bluetooth connection</div><DashboardLayout {...props} /></>;
}
