...
- Advanced memory write can select and filter the current Scan tags results, or accept an EPC manually. Selection fills the actual `epc` field sent with `WE` or `WD`; it does not alter the stored scan results. Those results are previous observations and are not a fresh presence check. Scan again after writing to verify the new data.
